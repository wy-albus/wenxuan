...
BASE_CHECKPOINT_PATH = ROOT / "models/checkpoints/active_store_experiment/lightgbm_v2_logl2_2m.txt"
EXTENDED_CHECKPOINT_PATH = ROOT / "models/checkpoints/active_store_wape/lightgbm_v2_logl2_active_store_wape_2m_2000.txt"
FINAL_MODEL_PATH = ROOT / "models/final/lightgbm_v2_logl2_active_store_wape_2m.txt"
CANDIDATE_PATH = ROOT / "reports/lightgbm_v2_active_store_wape_candidates_2m.csv"
CURVE_PATH = ROOT / "reports/lightgbm_v2_active_store_wape_curve_2m.png"
TEST_PREDICTION_PATH = ROOT / "data/outputs/lightgbm_v2_active_store_wape_2m_test_predictions.parquet"
FAIR_COMPARISON_PATH = ROOT / "reports/lightgbm_v2_active_store_wape_2m_fair_comparison.csv"
REPORT_PATH = ROOT / "reports/lightgbm_v2_active_store_wape_2m_report.md"
OLD_PREDICTION_PATH = ROOT / "data/outputs/lightgbm_v2_logl2_test_predictions.parquet"
LOG_DIR = ROOT / "logs/lightgbm/active_store_wape_2m"
TEMP_DIR = ROOT / "data/temp/active_store_wape_2m"

HORIZON = "2m"
MAX_ITERATION = 2000
EXPECTED_VALID_ROWS = 18_491_375
EXPECTED_TEST_ROWS = 12_859_664
...
def candidate_iterations() -> list[int]:
    return [100, 300, 500, 700, 850, 1000, 1200, 1400, 1600, 1800, 2000]
...
def output_paths() -> dict[str, Path]:
    return {
        "checkpoint": EXTENDED_CHECKPOINT_PATH,
        "final_model": FINAL_MODEL_PATH,
        "candidates": CANDIDATE_PATH,
        "curve": CURVE_PATH,
        "test_predictions": TEST_PREDICTION_PATH,
        "fair_comparison": FAIR_COMPARISON_PATH,
        "report": REPORT_PATH,
    }
...
def validate_candidate_rows(rows: list[dict[str, Any]], expected_count: int) -> None:
    ...
    if set(iterations) != set(extended_candidate_iterations()):
        raise ValueError("Candidate table does not cover the approved 100-2000 search points")
...
def setup_logging() -> tuple[logging.Logger, Path]:
    ...
    logger = logging.getLogger("wenxuan_active_store_wape_2m")
...
def _extended_checkpoint_metadata(base_metadata: dict, params: dict, training_summary: dict) -> dict:
    ...
    metadata.update(
        experiment="active_store_wape_2m_formal",
...
def train_to_2000(logger: logging.Logger) -> tuple[lgb.Booster, dict]:
    ...
    _, base_metadata = load_model_bundle(BASE_CHECKPOINT_PATH)
    features = pipeline.horizon_features(config, HORIZON)
    category_maps = base_metadata["category_maps"]
    guard = pipeline.MemoryGuard("active-store-wape-2m-train", logger)
    ...
    params = _training_params({"params": pipeline.training_params(config)})
    logger.info(
        "Training 2M from scratch to 2000 with train=%s valid=%s features=%d",
    ...
    train_set, valid_set = build_training_datasets(
    ...
    if booster.current_iteration() != MAX_ITERATION:
        raise RuntimeError(f"Training stopped at {booster.current_iteration()} instead of {MAX_ITERATION}")
    training_summary = {
        "base_iterations": 0,
    ...
    logger.info(
        "Saved 2M 2000-iteration checkpoint; sampling %.1fs training %.1fs peak %.2f GiB",
        sampling_seconds, training_seconds, guard.peak / (1024 ** 3),
    )
    del samples, train, valid, train_set, valid_set, booster
    gc.collect()
...
def _save_curve(frame: pd.DataFrame) -> None:
    ...
    axis.set_title("LightGBM V2 Active-Store 2M WAPE Search")
...
def select_on_complete_valid(
    ...
    if CANDIDATE_PATH.exists():
        ...
        pipeline = pipeline_module()
        guard = pipeline.MemoryGuard("active-store-wape-2m-valid", logger)
    ...
    if FINAL_MODEL_PATH.exists():
        ...
        if int(saved_metadata.get("best_iteration", -1)) != best_iteration:
            raise RuntimeError("Existing final 2M WAPE model conflicts with current candidate selection")
    else:
...
def evaluate_test_once(model: lgb.Booster, metadata: dict, logger: logging.Logger) -> tuple[dict, dict]:
    ...
    experiment = experiment_module()
    config = load_feature_config()
    guard = pipeline.MemoryGuard("active-store-wape-2m-test", logger)
    columns = experiment._physical_eval_columns(config, metadata["feature_names"], "2m")
    ...
    try:
        for number, frame in enumerate(
            experiment.iter_horizon_row_groups(DATASET_PATH, "2m", columns, {"test"}), start=1
        ):
            target = clip_target(frame["future_qty_2m"].to_numpy())
            ...
            output = pd.DataFrame(
                {
                    ...
                    "target_qty_2m": target.astype("float32"),
                    "lightgbm_v2_active_store_wape_pred_2m": prediction.astype("float32"),
                }
            )
    ...
    try:
        frame = connection.execute(
            ...
            WITH common AS MATERIALIZED (
              SELECT n.target_qty_2m AS target,
                     n.lightgbm_v2_active_store_wape_pred_2m AS new_pred,
                     o.lightgbm_v2_logl2_pred_2m AS old_pred,
                     ABS(n.target_qty_2m-o.target_qty_2m) AS target_difference
...
def write_report(candidate_frame: pd.DataFrame, metadata: dict, test_payload: dict, fair: pd.DataFrame, log_path: Path) -> None:
    ...
    lines = [
        ...
        "## Training And Search", "",
        f"- Formal training starts from scratch; training/search limit: {MAX_ITERATION} trees.",
        f"- Train sample: {int(training.get('train_rows', 0)):,}; sampled Valid: {int(training.get('sampled_valid_rows', 0)):,}.",
        f"- Sampling: {float(training.get('sampling_seconds', 0)):.1f}s; formal training: {float(training.get('formal_training_seconds', 0)):.1f}s.",
...
def run() -> dict:
    ...
    started = time.perf_counter()
    booster, extended_metadata = train_to_2000(logger)
    ...
    elapsed = time.perf_counter() - started
    logger.info("Active-store WAPE 2M experiment completed in %.1fs", elapsed)
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and select LightGBM V2 active-store 2M by complete Valid WAPE")
    parser.parse_args()
    result = run()
