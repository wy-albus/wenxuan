...
from .inference_feature_service import InferenceFeatureService
...
class PredictionService:
    def __init__(self, model_registry: ModelRegistry | None = None) -> None:
        ...
        self.inference_features = InferenceFeatureService(model_contracts=None)
    ...
    def run_future(
        ...
        *,
        ...
        run_id = prediction_run_id or uuid.uuid4().hex
        if job_service and job_id:
            job_service.update(job_id, status="RUNNING", progress=15)
            job_service.log(job_id, f"building inference features for target {target_month}")
        built = self.inference_features.build(model_ids=model_ids, target_month=target_month, prediction_run_id=run_id)
        selected = add_runtime_columns(built.features)
        if job_service and job_id:
            job_service.update(job_id, status="RUNNING", progress=35)
            job_service.log(job_id, f"built {len(selected)} inference rows for {built.metadata['observation_month']}")
        ...
        for index, model_id in enumerate(model_ids, start=1):
            ...
            pred_raw, pred_int, pred_mc = apply_prediction_postprocess(combine_predictions(p_sale, conditional_qty))
            output = pd.DataFrame({
                "dataset_id": "standard-history", "prediction_run_id": run_id, "model_id": model_id,
                "site_no": selected["site_no"].astype(str).to_numpy(), "item_id": selected["item_id"].astype(str).to_numpy(),
                "p_sale": np.asarray(p_sale, dtype="float64"), "conditional_qty": np.asarray(conditional_qty, dtype="float64"),
                "pred_qty_raw": pred_raw, "pred_qty_int": pred_int, "pred_mc": pred_mc,
            })
            ...
            result_parts.append(output)
            if job_service and job_id:
                job_service.update(job_id, status="RUNNING", progress=35 + int(index / len(model_ids) * 50))
                job_service.log(job_id, f"completed {model_id} future prediction")
        ...
        summary = {
            "prediction_run_id": run_id, "dataset_id": "standard-history",
            "observation_month": built.metadata["observation_month"], "target_month": target_month,
            "data_cutoff_month": built.metadata["data_cutoff_month"],
            "model_ids": model_ids, "model_summaries": model_summaries,
            "provenance": built.metadata,
        }
        summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
        if job_service and job_id:
            job_service.log(job_id, f"wrote prediction files to {prediction_dir}")
        return {
            "prediction_run_id": run_id, "prediction_dir": str(prediction_dir), "observation_month": built.metadata["observation_month"],
            "target_month": target_month, "data_cutoff_month": built.metadata["data_cutoff_month"],
            "inference_feature_path": str(built.path) if built.path else None, "provenance": built.metadata,
            "output_files": [str(prediction_path), str(summary_path), str(top_books_path), str(store_summary_path)],
            "summary": summary,
        }
